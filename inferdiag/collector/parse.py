...
def _quantile(snapshot: dict[str, list[dict]], target: str, q: str) -> float | None:
    """取裸名为 target、且带 quantile=q 标签的样本值。"""
...
def normalize(snapshot: dict[str, list[dict]], engine: str = "auto") -> Sample:
    ...
    s.ttft_p50_ms = sec_to_ms(_quantile(snapshot, "time_to_first_token_seconds", "0.5"))
    s.ttft_p99_ms = sec_to_ms(_quantile(snapshot, "time_to_first_token_seconds", "0.99"))
    tpot = _pick(snapshot, "time_per_output_token_seconds")
    s.tpot_ms = None if tpot is None else round(tpot * 1000, 2)
    s.e2e_p50_ms = sec_to_ms(_quantile(snapshot, "e2e_request_latency_seconds", "0.5"))
    s.e2e_p99_ms = sec_to_ms(_quantile(snapshot, "e2e_request_latency_seconds", "0.99"))
    ...
    s.prefix_cache_hit_pct = _pick(snapshot, "prefix_cache_hit_perc")
